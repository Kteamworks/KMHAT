...
//     }
    ...
    if ($GLOBALS['rx_enable_SLN']) {
        ...
        function multiprintcss_header($p) {
	       ...
	       // echo ("<td></td>\n");
	        echo ("<td>\n");
                echo ("<img WIDTH='1100pt' src='./interface/pic/medii_rx.jpg' />");
                echo ("</td>\n");
				echo ("<td></td>\n");
	        echo ("</tr>\n");
	        //echo ("<tr>\n");
		 ...
		 echo("</td>\n");
            ...
            $res = sqlQuery("SELECT district as addr,housename,place,sex,age from patient_data where pid =". mysql_real_escape_string ($p->patient->id));    
			$patterns = array ('/\n/');
	        $replace = array ('<br>');
	        $res = preg_replace($patterns, $replace, $res);
			$dis=sqlStatement("SELECT title from list_options where list_id='District' and option_id='".$res['addr']."'");
			$dis1=sqlFetchArray($dis);
                echo ('<span class="small">'.$res['housename'].','.$res['place'].','.  $dis1['title'].'</span>');
	        echo ("</td>\n");
			echo("</tr>\n");
			echo ("<tr>\n");
	        echo ("<td >\n");

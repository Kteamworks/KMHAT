...
require_once("C:/xampp/htdocs/mhat/interface/tcpdf/tcpdf.php");
...
$query_pt_data = "SELECT * from patient_data WHERE pid=".$pid_da;
...
// Print text using writeHTMLCell()
    ...
    $pdf->Output('C:/xampp/htdocs/'.$pid_da.'_prescription.pdf', 'F');
		    $mail = new MyMailer();
...
$querye = "SELECT email FROM facility WHERE id=".$pfacility;
$res_em = mysql_query($querye);
// $rows = array();
while($row1 = mysql_fetch_array($res_em)) {
 /*   $rows[] = $row1;
}
...
$pid_da = $patient['patient_id'];
	$mail->AddAttachment("C:/xampp/htdocs/".$pid_da."_prescription.pdf");	
 // }	
    if ($mail->Send()) {
        return true;
    } else {

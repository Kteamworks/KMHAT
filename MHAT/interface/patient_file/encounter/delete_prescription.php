...
if ($_POST['confirm']) {
    // set the delete flag of the indicated form
	 sqlStatement("update prescriptions set deleted=1,deleted_user='".$_SESSION['authUser']."' where id='".$id."'");
    // log the event   
...
<span class="title">Delete Prescription</span>

<form method="post" action="<?php echo $rootdir;?>/patient_file/encounter/delete_prescription.php" name="my_form" id="my_form">
<?php
...
<p>
<?php
$pres_id=sqlStatement("SELECT * from prescriptions where id='".$id."'");
$pres_id1=sqlFetchArray($pres_id);
$pres_id2=$pres_id1['drug'];
?>
You are about to delete a Prescription: '<?php echo attr($pres_id2);?>'
</p>
<table>
<?php
...
function ConfirmDelete() {
	 $.jAlert({'type': 'confirm','confirmQuestion':'Are you sure you wish to Delete this Prescription? ', 'onConfirm': function(){
        top.restoreSession();
        $("#my_form").submit();
